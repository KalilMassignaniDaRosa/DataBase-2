IF NOT EXISTS(SELECT * FROM sys.databases WHERE name = 'atividadeSimposio')
BEGIN
...
CREATE TABLE Pessoa(
	...
	tipo_pessoa VARCHAR(12) NOT NULL DEFAULT 'Participante' CHECK (tipo_pessoa IN ('Participante', 'Palestrante', 'Organizador')),
    -- Impedindo que eh_universitario seja true e o codigo_universitario esteja vazio
    CONSTRAINT chk_codigo_universitario
		CHECK(
			(eh_universitario = 1 AND codigo_universitario IS NOT NULL)
            OR 
            (eh_universitario = 0 AND codigo_universitario IS NULL)
        )
);
